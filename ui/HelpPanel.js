...
const HelpPanel = (() => {
  ...
  const TABS_REFERENCE = [
    ...
    },
    {
      ...
      items: [
        ['Beat detector', 'Analyses the audio in real time using spectral flux onset detection. Outputs a beat pulse (iBeat), per-band beats (kick, snare, hi-hat), BPM estimate, and phrase tracking (beat 1–4, bar 1–4).'],
        ['Sensitivity sliders', 'Flux sensitivity: how much louder than the noise floor a transient needs to be to register as a beat. Higher = less sensitive. Min interval: minimum gap between detected beats in ms.'],
        ['Auto-calibrate', 'Click "Auto-calibrate (8s)", play your audio, and wait. Vael listens for 8 seconds and sets all thresholds based on what it actually hears. Best run during a representative section of your track.'],
        ['Tap tempo', 'Press T (or tap the TAP button in performance mode) at least 3 times to set BPM manually. Averages the last 8 taps.'],
        ['MIDI clock', 'Go to the MIDI tab to see if an incoming MIDI clock signal is being received. When locked, the MIDI clock BPM overrides the beat detector.'],
        ['Signal meters', 'Live readout of every signal Vael computes: RMS, spectral centroid, flux, per-band energy. Watch these while audio plays to understand what values your ModMatrix routes will receive.'],
      ],
    },
    {
      tab: 'VIDEO',
